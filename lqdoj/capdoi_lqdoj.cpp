...
signed main() {
    ...
    for (int i = 0; i < N; ++i) {
        ll pos_fi = lower_bound(lst.begin() + i + 1, lst.begin() + N, K - lst[i]) - lst.begin();
        ll pos_la = upper_bound(lst.begin() + i + 1, lst.begin() + N, K - lst[i]) - lst.begin();
        result += pos_la - pos_fi;
    }
    cout << result;
    return 0; 
}